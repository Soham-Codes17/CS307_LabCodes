...
def expected_return_problem3(state, action, value):
    """
    Calculate expected return for Problem 3 with free moves and parking costs
    """
    bikes1, bikes2 = state
    
    # Apply movement with bounds checking
    bikes1_after_move = max(0, min(bikes1 - action, MAX_BIKES))
    bikes2_after_move = max(0, min(bikes2 + action, MAX_BIKES))
    
    # Movement cost with free move adjustment
    if action > 0:  # Moving from location 1 to 2
        move_cost = MOVE_COST * max(0, action - FREE_MOVE)  # First bike free
    else:  # Moving from location 2 to 1
        move_cost = MOVE_COST * abs(action)  # All bikes cost
    
    # Parking cost
    parking_penalty = 0
    if bikes1_after_move > PARKING_LIMIT:
        parking_penalty += PARKING_COST
    if bikes2_after_move > PARKING_LIMIT:
        parking_penalty += PARKING_COST
    
    total_reward = -move_cost - parking_penalty
...
def policy_iteration_problem3():
    """
    Policy iteration algorithm for Problem 3
    """
    ...
    print("Starting Policy Iteration for Problem 3...")
    print("Special Conditions:")
    print("  • First bike moved 1→2 is FREE")
    print("  • Parking cost: INR 4 if bikes > 10 at any location")
    print("  • All moves 2→1 cost INR 2 per bike")
        ...
        while True:
            ...
            for i in range(MAX_BIKES + 1):
                for j in range(MAX_BIKES + 1):
                    old_val = value[i, j]
                    action = policy[i, j]
                    new_value[i, j] = expected_return_problem3((i, j), action, value)
                ...
                for action in actions:
                    ...
                    if action >= 0:  # Moving from loc1 to loc2
                        if action <= min(i, MAX_MOVE) and (j + action) <= MAX_BIKES:
                            action_val = expected_return_problem3((i, j), action, value)
...
def analyze_policy_problem3(policy, value):
    """
    Analyze the optimal policy for Problem 3
    """
    print("\n" + "="*60)
    print("PROBLEM 3 POLICY ANALYSIS")
    print("="*60)
    ...
    print(f"\nMove Directions:")
    print(f"Location 1 → Location 2: {positive_moves} states ({positive_moves/441*100:.1f}%)")
    print(f"Location 2 → Location 1: {negative_moves} states ({negative_moves/441*100:.1f}%)")
    print(f"No movement: {zero_moves} states ({zero_moves/441*100:.1f}%)")
    
    # Parking constraint analysis
    print(f"\nParking Constraint Analysis (Limit: {PARKING_LIMIT} bikes):")
    parking_violations = 0
    for i in range(MAX_BIKES + 1):
        for j in range(MAX_BIKES + 1):
            action = policy[i, j]
            bikes1_after = i - action
            bikes2_after = j + action
            if bikes1_after > PARKING_LIMIT or bikes2_after > PARKING_LIMIT:
                parking_violations += 1
    
    print(f"Total states with potential parking cost: {parking_violations}/441 ({parking_violations/441*100:.1f}%)")
    
    # Free move utilization
    free_move_states = np.sum(policy == 1)
    print(f"\nFree Move Utilization:")
    print(f"States using exactly 1 bike transfer (free): {free_move_states}/441")
...
if __name__ == "__main__":
    ...
    policy, value = policy_iteration_problem3()
    
    total_time = time.time() - start_time
    print(f"\nTotal execution time: {total_time:.2f}s ({total_time/60:.2f} minutes)")
    
    analyze_policy_problem3(policy, value)
