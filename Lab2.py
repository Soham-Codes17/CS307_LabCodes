...
Install all necessary packages req to run this file
...
def detect_plagiarism_from_pdfs(pdf1_path, pdf2_path, output_csv="plagiarism_report.csv", threshold=10):
    ...
    if not text1:
        print(f" Warning: No extractable text found in {pdf1_path}.")
        return
    if not text2:
        print(f" Warning: No extractable text found in {pdf2_path}.")
        return
    ...
    if len(sentences1) == 0 or len(sentences2) == 0:
        print(" No valid sentences extracted from one or both documents. Cannot run alignment.")
        return
